...
const Login = () => {
    ...
    const [siretStatus, setSiretStatus] = useState({ isValid: false, message: null, type: null });
    ...
    const [formData, setFormData] = useState({
        ...
        lastName: '', phone: '', companyName: '', companyAddress: '', siret: '', tvaNumber: ''
    });
    ...
    const handleSiretChange = async (e) => {
        ...
        setFormData(prev => ({ 
            ...
            ...(value.length < 14 && { companyName: '', companyAddress: '', tvaNumber: '' })
        }));
        ...
        if (value.length === 14) {
            ...
            try {
                const response = await fetch(`https://recherche-entreprises.api.gouv.fr/search?q=${value}`);
                const data = await response.json();

                const etablissement = data.results && data.results.find(r => r.siren === value.substring(0, 9));

                if (etablissement) {
                    setSiretStatus({ isValid: true, message: "✅ Entreprise trouvée !", type: 'success' });

                    const newAddress = etablissement.siege.geo_adresse || etablissement.siege.adresse;
                    ...
                    setSiretStatus({ isValid: false, message: "❌ SIRET introuvable ou fermé.", type: 'error' });
                    // On vide si introuvable
                    setFormData(prev => ({ ...prev, companyName: '', companyAddress: '', tvaNumber: '' }));
                }
            } catch (error) {
                console.error("Erreur API SIRET", error);
                setSiretStatus({ isValid: false, message: "⚠️ Erreur vérification (Réessayez)", type: 'error' });
            }
    ...
    const validateForm = () => {
        ...
        if (isSignUp) {
            ...
            else if (!/^\d{10}$/.test(formData.phone)) newErrors.phone = "10 chiffres requis";

            // Vérification simple car champs bloqués remplis par le SIRET
            if (!formData.companyName) newErrors.companyName = "Requis (Entrez un SIRET valide)";
            if (!formData.companyAddress) newErrors.companyAddress = "Requis (Entrez un SIRET valide)";
            ...
            if (response.ok) {
                if (isSignUp) {
                    ...
                    setFormData({
                        ...
                        companyName: '', firstName: '', lastName: '', phone: '', siret: '', tvaNumber: ''
                    });
                    setSiretStatus({ isValid: false, message: null, type: null });
                    window.scrollTo(0, 0);
                } else {
                    login(data.token, data.userId, { firstName: data.firstName, companyName: data.companyName }, data.role);
    ...
    const EyeIcon = ({ visible }) => (
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#666" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            {visible ? <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"><circle cx="12" cy="12" r="3"></circle></path> : <path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path>}
        ...
        return (
            <div className="login-page">
                <div className="login-card validation-screen">
                    ...
                    <p>Un lien de validation a été envoyé à <strong>{pendingEmail}</strong>.</p>
                    ...
                    <div className="resend-section">
                        <button className="resend-btn" onClick={handleResendEmail} disabled={resendTimer > 0 || isResending}>
                            {isResending ? "Envoi..." : resendTimer > 0 ? `Renvoyer dans ${resendTimer}s` : "Renvoyer l'email"}
                        </button>
                    </div>
                    <button className="back-link" onClick={() => { setWaitingForValidation(false); setIsSignUp(false); }}>Retour à la connexion</button>
                </div>
            </div>
        );
    }
            ...
            <div className="login-card">
                ...
                <form onSubmit={handleSubmit} autoComplete="off">
                    {isSignUp && (
                        <div className="form-row">
                            <div className="form-col">
                                <label>Prénom</label>
                                <input type="text" name="firstName" placeholder="Jean" value={formData.firstName} onChange={handleChange} className={errors.firstName ? 'has-error' : ''} />
                                {errors.firstName && <span className="error-msg">Requis</span>}
                            </div>
                            <div className="form-col">
                                <label>Nom</label>
                                <input type="text" name="lastName" placeholder="Dupont" value={formData.lastName} onChange={handleChange} className={errors.lastName ? 'has-error' : ''} />
                    ...
                    <div className="form-group">
                        ...
                        <input type="email" name="email" placeholder="contact@entreprise.com" value={formData.email} onChange={handleChange} autoComplete="username" className={errors.email ? 'has-error' : ''} />
                    ...
                    {isSignUp && (
                        <div className="form-group">
                            ...
                            <input type="tel" name="phone" placeholder="0612345678" maxLength="10" value={formData.phone} onChange={handleChange} className={errors.phone ? 'has-error' : ''} />
                        ...
                        <>
                            <div className="form-row">
                                <div className="form-col" style={{flex: 1}}>
                                    ...
                                    {siretStatus.message && (
                                        <div style={{fontSize: '0.8rem', marginTop: '5px', color: siretStatus.type === 'success' ? '#27ae60' : '#e74c3c'}}>
                                            {siretStatus.message}
                                        </div>
                                    )}
                                </div>
                    ...
                    <div className="form-group">
                        ...
                        <div style={{ position: 'relative' }}>
                            <input type={showPassword ? "text" : "password"} name="password" placeholder="8 car. min" value={formData.password} onChange={handleChange} style={{ paddingRight: '45px' }} autoComplete={isSignUp ? "new-password" : "current-password"} className={errors.password ? 'has-error' : ''} />
                            <div className="eye-icon" onClick={() => setShowPassword(!showPassword)}><EyeIcon visible={showPassword} /></div>
                        </div>
                    ...
                    {isSignUp && (
                        <div className="form-group">
                            ...
                            <input type="password" name="confirmPassword" placeholder="Répétez" value={formData.confirmPassword} onChange={handleChange} autoComplete="new-password" className={errors.confirmPassword ? 'has-error' : ''} />
                            {errors.confirmPassword && <span className="error-msg">{errors.confirmPassword}</span>}
                        </div>
                    )}

                    <button type="submit" className="submit-btn">{isSignUp ? "Créer mon compte" : "Se connecter"}</button>
                </form>

                <div className="toggle-text">
                    {isSignUp ? "Vous avez déjà un compte ?" : "Pas encore de compte professionnel ?"}
                    <span className="link" onClick={() => { setIsSignUp(!isSignUp); setErrors({}); setGlobalError(null); setSuccessMessage(null); }}>
