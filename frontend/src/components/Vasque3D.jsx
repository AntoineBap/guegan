...
const Vasque3D = ({ config }) => {
  ...
  const apronH =
    (config.aprons && config.apronHeight ? config.apronHeight : 40) /
    UNIT_SCALE;
  ...
  const calculatedSinks = useMemo(() => {
    const sinks = config.sinks || [
      {
        id: 999,
        type: config.sink || "Aucune cuve",
        position: config.position || "center",
        offset: config.sinkOffset || 100,
      },
    ];
    ...
    const items = sinks.map((s) => {
      ...
      const offsetVal =
        s.offset !== undefined && s.offset !== null
          ? s.offset / UNIT_SCALE
          : GAP_DEFAULT;
  ...
  const planComponents = useMemo(() => {
    ...
    if (hasAnyDrainer) {
      ...
      const geomFull = new THREE.ExtrudeGeometry(shapeTop, {
        ...extrudeSettings,
        depth: thickness,
      });
        ...
        for (let i = 0; i < count; i++) {
          ...
          visuals.push(
            <mesh
              key={`${idx}-${i}`}
              geometry={geometry}
              position={[drainCenter, 0.401, currentZ]}
              rotation={[0, rotY, 0]}
            >
  ...
  const SingleSinkGeometry = ({ s }) => {
    ...
    const { outerWallGeom, innerSkinGeom, tubesGeometry, cornerGeometry } =
      useMemo(() => {
        ...
        outerShape.moveTo(-wOut / 2 + rOut, -hOut / 2);
        outerShape.lineTo(wOut / 2 - rOut, -hOut / 2);
        outerShape.absarc(
          wOut / 2 - rOut,
          -hOut / 2 + rOut,
          rOut,
        ...
        outerShape.lineTo(wOut / 2, hOut / 2 - rOut);
        outerShape.absarc(
          wOut / 2 - rOut,
          hOut / 2 - rOut,
          rOut,
        ...
        holeIn.moveTo(-hwIn / 2, -hhIn / 2 + rIn);
        holeIn.lineTo(-hwIn / 2, hhIn / 2 - rIn);
        holeIn.absarc(
          -hwIn / 2 + rIn,
          hhIn / 2 - rIn,
          rIn,
        ...
        holeIn.lineTo(hwIn / 2 - rIn, hhIn / 2);
        holeIn.absarc(
          hwIn / 2 - rIn,
          hhIn / 2 - rIn,
          rIn,
    ...
    return (
      <group position={[s.x, 0, s.z]}>
        ...
        {s.hasTapHole && (
          <mesh
            position={[tapX, 0.401, -s.height / 2 - 0.4]}
            rotation={[-Math.PI / 2, 0, 0]}
          >
            <circleGeometry args={[0.175, 64]} />
  ...
  const materialProps = {
    color: config.color === "white" ? "#ffffff" : "#111111",
    roughness: 0.9,
    metalness: 0.1,
  };
  ...
  return (
    <group position={[0, elevationY, 0]}>
      {planComponents.isSplit ? (
        <>
          <mesh position={[0, 0, 0]} geometry={planComponents.bottom}>
            <meshStandardMaterial {...materialProps} />
          </mesh>
          <mesh
            position={[0, 0.4 - grooveDepth, 0]}
        ...
        <mesh position={[0, 0, 0]} geometry={planComponents.full}>
          <meshStandardMaterial {...materialProps} />
        </mesh>
      )}

      {calculatedSinks.map((s, i) => (
        <SingleSinkGeometry key={i} s={s} />
      ))}
      {drainerVisuals}
      ...
      {config.aprons && (
        <>
          {config.apronFront && (
            <mesh
              position={[0, 0.4 - apronH / 2, totalW / 2 - wallThickness / 2]}
            >
          ...
          {config.apronLeft && (
            <mesh
              position={[-totalL / 2 + wallThickness / 2, 0.4 - apronH / 2, 0]}
            >
          ...
          {config.rimLeft && (
            <mesh
              position={[
                ...
                0.4 + config.rimHeigh / UNIT_SCALE / 2,
                0,
              ]}
            >
              <boxGeometry
      ...
      {config.splashback && (
        <>
          {geomFront && (
            <mesh
              position={[0, 0.4, totalW / 2 - splashRadius]}
              geometry={geomFront}
            >
              <meshStandardMaterial {...materialProps} />
            </mesh>
          ...
          {geomLeft && (
            <mesh
              position={[-totalL / 2 + splashRadius, 0.4, 0]}
